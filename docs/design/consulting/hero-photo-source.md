...
The hero uses the existing generated Ross visualization with Paper 0.0.80's `halftoneCmykFragmentShader`, its noise texture, and vanilla `ShaderMount` API. `rossSceneShader.ts` extends the fragment shader with localized tree-canopy displacement, ten moving vehicles sampled from the source image, and twelve pedestrians following the walkways. Walkers have direction-aware bodies, alternating steps, arm movement, and slightly clearer silhouettes and shadows at the overhead scale. Curbside vehicles remain parked. Vehicles follow continuous rounded turns into the upper street, with loops occurring outside the frame. The camera and building geometry remain fixed; no new image generation or video asset is involved.

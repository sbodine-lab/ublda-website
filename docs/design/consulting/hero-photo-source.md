...
The hero uses the existing generated Ross visualization with Paper 0.0.80's `halftoneCmykFragmentShader`, its noise texture, and vanilla `ShaderMount` API. `rossSceneShader.ts` extends the fragment shader with localized tree-canopy displacement, ten moving vehicles sampled from the source image, and twenty-four pedestrians following the walkways on both sides of the building. Walkers have direction-aware bodies, alternating steps, arm movement, and slightly clearer silhouettes and shadows at the overhead scale. Curbside vehicles remain parked. Vehicles follow continuous rounded turns aligned with the upper street, with loops occurring outside the frame. A pavement mask clips car bodies and shadows to the road. A source-image foliage mask, sampled after tree sway and before CMYK tinting, keeps cars and walkers behind overhanging green/gold canopy. Added west-side routes use the plaza, paths, and courtyard rather than roof or lawn areas. The camera and building geometry remain fixed; no new image generation or video asset is involved.

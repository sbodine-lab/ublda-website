...
Rendered using the actual Paper Design `FlutedGlass` shader from `@paper-design/shaders-react` 0.0.80 in Chrome WebGL2 at 1333 × 1000. Settings: lines, prism, size 0.95, distortion 0.2, shadows 0.12, highlights 0.12, angle/shift/stretch/blur/edges/grain/speed 0, fit cover, background #f8f7f3, shadow #0a3658, highlight #ffffff. Exported at WebP quality 88. The glass treatment is baked into the asset; the hero loads it with high priority as a normal image, adding no runtime shader bundle, GPU context, or animation. The original photo remains unchanged.

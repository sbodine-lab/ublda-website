...
parent, width = line('UBLDA', 'dm-sans-medium.ttf', 48, 36, .25)
service, service_width = line('Consulting', 'dm-sans-regular.ttf', 32, 71, -.1)
for variant, parent_color, service_color in [('color', '#0A3658', '#3559A8'), ('white', '#FAF9F6', '#FAF9F6')]:
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {round(max(width, service_width) + 2, 2)} 80" role="img" aria-labelledby="title"><title id="title">UBLDA Consulting</title><path fill="{parent_color}" d="{parent}"/><path fill="{service_color}" d="{service}"/></svg>'''

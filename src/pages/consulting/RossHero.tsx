...
import { createRossSceneShader, rossSceneSpeed } from "./rossSceneShader";
...
export function RossHero() {
  ...
  useEffect(() => {
    ...
    // Drift the printing grid on its own slow 48-second orbit.
    ...
    const tick = (now: number) => {
      ...
      elapsed += delta;
      sceneSpeed += (targetSpeed - sceneSpeed) * (1 - Math.exp(-delta / 650));
      sceneTime += delta * sceneSpeed / 1000;
      previous = now;
      if (mount && now - painted >= 1000 / 30) {
        const phase = elapsed * Math.PI * 2 / 48000;
        const wave = Math.sin(phase);
        mount.setUniforms({
          u_sceneTime: sceneTime,
          u_sceneBlend: 0.52 + wave * 0.06,
          u_ubldaGridDrift: [Math.sin(phase) * 8, (Math.cos(phase) - 1) * 5],
          u_gainC: 0.12 + wave * 0.06, u_gainM: -wave * 0.04,
        });
        host.dataset.sceneSpeed = sceneSpeed.toFixed(2);
        painted = now;
      }
    ...
    const scroll = () => {
      const hero = host.closest(".st-hero");
      if (!hero) return;
      const rect = hero.getBoundingClientRect();
      targetSpeed = rossSceneSpeed(-rect.top / Math.max(1, rect.height * 0.85));
    };
    const resize = () => {
      mount?.setMaxPixelCount(mobile.matches ? 900_000 : 2_200_000);
      // Match object-position:88% on phones so the street remains in frame.
      const visibleWidth = Math.min(1, host.clientWidth / Math.max(1, host.clientHeight) / (1672 / 941));
      mount?.setUniforms({ u_cropShift: mobile.matches ? (1 - visibleWidth) * 0.38 : 0 });
      scroll();
    };
    ...
    void (async () => {
      try {
        ...
        await noise?.decode();
        if (disposed) return;
        const fragment = createRossSceneShader(paper.halftoneCmykFragmentShader);
        mount = new paper.ShaderMount(host, fragment, {
          u_ubldaGridDrift: [0, 0],
          u_sceneTime: 0, u_sceneBlend: 0.52, u_cropShift: 0,
          u_image: photo,

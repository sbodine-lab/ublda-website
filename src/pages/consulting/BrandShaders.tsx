...
export function RossHalftone({ source = 'photo', size = 0.3, gold = true }: { source?: RossSource; size?: number; gold?: boolean }) {
  return <HalftoneCmyk
    image={source === 'illustration' ? '/ross-illustration.png' : '/ross-building.jpg'}
...
export function AnimatedMark({ effect, paused = false }: { effect: LogoEffect; paused?: boolean }) {
  const common = { image: '/logo-mark.svg', colorBack: '#FAF9F6', scale: 1.65, fit: 'contain' as const, minPixelRatio: 1, maxPixelCount: 650000, style: { width: '100%', height: '100%' }, speed: paused ? 0 : 0.45, frame: 8000 }

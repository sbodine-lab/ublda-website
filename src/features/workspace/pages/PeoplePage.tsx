  ...
  return (
    <LeadershipPage className="ws-people-page">
      <LeadershipSurface className="leadership-directory-surface" flush>
        ...
        {visible.length ? <div className="ws-people-list">{visible.map((person) => <button type="button" className="ws-person-row" onClick={() => isAdmin && setEditing(person)} key={person.memberId}><Avatar><AvatarImage src={person.avatarUrl} /><AvatarFallback>{initials(person.displayName)}</AvatarFallback></Avatar><div><strong>{person.displayName}</strong><span>{person.clubRole}</span></div><span className="ws-person-team">{person.team}</span></button>)}</div> : <Empty className="ws-empty ws-page-empty"><EmptyHeader><EmptyMedia variant="icon"><Users /></EmptyMedia><EmptyTitle>no people found</EmptyTitle><EmptyDescription>Try a different search or directory view.</EmptyDescription></EmptyHeader></Empty>}
      </LeadershipSurface>

...
export function ClubCalendarPage() {
  ...
  return (
    <div className="ws-page">
      <header className="ws-page-header ws-page-header-row"><div><p className="ws-kicker">club calendar</p><h1>calendar</h1></div>{isAdmin && <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="ws-primary-action"><CalendarPlus /> add event</Button></DialogTrigger><DialogContent className="ws-dialog"><DialogHeader><DialogTitle>add event</DialogTitle></DialogHeader><form onSubmit={submit}><FieldGroup><Field><FieldLabel htmlFor="event-title">title</FieldLabel><Input id="event-title" name="title" required autoFocus /></Field><div className="ws-form-grid"><Field><FieldLabel>type</FieldLabel><Select value={eventType} onValueChange={(value) => setEventType(value as ClubEventType)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{["meeting", "event", "deadline", "project"].map((value) => <SelectItem value={value} key={value}>{value}</SelectItem>)}</SelectContent></Select></Field><Field><FieldLabel htmlFor="event-location">location</FieldLabel><Input id="event-location" name="location" /></Field></div><div className="ws-form-grid"><Field><FieldLabel htmlFor="event-start">starts</FieldLabel><Input id="event-start" name="startAt" type="datetime-local" defaultValue={todayDateInput()} required /></Field><Field><FieldLabel htmlFor="event-end">ends</FieldLabel><Input id="event-end" name="endAt" type="datetime-local" /></Field></div><div className="ws-form-grid"><Field><FieldLabel htmlFor="event-owner">owner</FieldLabel><select id="event-owner" name="ownerMemberId" className="ws-native-select"><option value="">unassigned</option>{people.map((person) => <option value={person.memberId} key={person.memberId}>{person.displayName}</option>)}</select></Field><Field><FieldLabel htmlFor="event-project">project</FieldLabel><select id="event-project" name="projectId" className="ws-native-select"><option value="">none</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></Field></div></FieldGroup><DialogFooter className="ws-dialog-footer"><Button type="submit">add event</Button></DialogFooter></form></DialogContent></Dialog> }</header>

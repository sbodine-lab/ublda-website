...
import { handleOperationsRequest } from '../server/operationsService.js'

const MAX_REQUEST_BYTES = 64 * 1_024
const allowedFetchSites = new Set(['same-origin', 'same-site'])

type OperationsHandlerOptions = {
  allowedOrigins?: Set<string>
  handleRequest?: typeof handleOperationsRequest
}
...
export default async function handler(
  ...
  if (!/^application\/json(?:\s*;|$)/.test(contentType)) {
    return res.status(415).json({ error: 'The Operations request must be JSON.' })
  }
  ...
  if (
    ...
    return res.status(413).json({ error: 'The Operations request is too large.' })
  }
  ...
  if (!approvedOrigin(origin, allowedOrigins) || (fetchSite && !allowedFetchSites.has(fetchSite))) {
    return res.status(403).json({ error: 'This Operations request came from an unapproved origin.' })
  }

  const result = await (options.handleRequest ?? handleOperationsRequest)(req.body, requestIp(req))
